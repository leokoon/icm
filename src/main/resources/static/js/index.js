(function ($) {
    ...
    $.learunindex = {
        ...
        getMenus: function () {
            var menuData;
            $.ajax({
                url: $.learunindex.getRootPath()+'/home/doSysMenu.do',
                type: 'get',
                dataType: "text",
                async:false,
        ...
        loadMenu: function (data) {
            ...
            $.each(data, function (i) {
                ...
                if (row.parentId == "0") {
                    ...
                    _html += '<a href="#">';
                    _html += '<i class="' + row.icon + '"></i><span>' + row.authName + '</span><i class="fa fa-angle-left pull-right"></i>';
                    _html += '</a>';
                    var childNodes = $.learunindex.jsonWhere(data, function (v) { return v.parentId == row.authId });
                    if (childNodes.length > 0) {
                        _html += '<ul class="treeview-menu">';
                        $.each(childNodes, function (i) {
                            var subrow = childNodes[i];
                            var subchildNodes = $.learunindex.jsonWhere(data, function (v) { return v.parentId == subrow.authId });
                            _html += '<li>';
                            if (subchildNodes.length > 0) {
                                _html += '<a href="#"><i class="' + subrow.icon + '"></i>' + subrow.authName + '';
                                _html += '<i class="fa fa-angle-left pull-right"></i></a>';
                                _html += '<ul class="treeview-menu">';
                                $.each(subchildNodes, function (i) {
                                    var subchildNodesrow = subchildNodes[i];
                                    _html += '<li><a class="menuItem" data-id="' + subrow.authId + '" href="' + subrow.authUrl + '"><i class="' + subchildNodesrow.icon + '"></i>' + subchildNodesrow.authName + '</a></li>';
                                });
                                _html += '</ul>';

                            } else {
                                _html += '<a class="menuItem" data-id="' + subrow.authId + '" href="' + subrow.authUrl + '"><i class="' + subrow.icon + '"></i>' + subrow.authName + '</a>';
                            }
                            _html += '</li>';
                        });
                        _html += '</ul>';
